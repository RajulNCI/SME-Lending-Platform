...
Mirrors the team HLD: on submit a job is enqueued; a worker runs OCR -> AI -> decision and
writes real status/progress here, which the borrower UI polls.
"""
import enum
import uuid
from datetime import datetime, timezone

from sqlalchemy import String, Integer, JSON, DateTime, ForeignKey, Enum as SAEnum, Text
...
STEPS = ["OCR_STARTED", "OCR_COMPLETED", "AI_EXTRACTION", "RISK_SCORING",
         "DECISION_GENERATED", "COMPLETED"]
...
class ProcessingJob(Base):
    ...
    steps_log:      Mapped[list | None] = mapped_column(JSON, nullable=True)  # [{step, at}]
