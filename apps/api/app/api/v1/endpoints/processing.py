"""
Async processing endpoints (team HLD: submit -> queue -> worker -> poll status).

POST /api/v1/applications/{id}/submit  -> create a job, enqueue, return immediately
...
async def submit_application(
    ...
    token_data: dict[str, Any] = Depends(
        require_roles("borrower_sme", "credit_officer", "ops_manager", "it_admin")
    ),
...
async def get_status(
    ...
    if not job:
        raise HTTPException(
            status_code=404, detail="No processing job for this application"
        )
    return StatusOut(
        application_id=application_id,
        job_id=job.id,
