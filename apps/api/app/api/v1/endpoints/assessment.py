...
POST /api/v1/applications/{id}/assess  → compute PD / grade / DSCR / IFRS9 / SHAP, persist,
move the application into the HITL queue, and return the full assessment.
"""
...
_FIELDS = ("sector", "years_trading", "annual_revenue", "net_profit", "total_assets",
...
async def run_assessment(
    application_id: str,
    token_data: dict = Depends(require_roles("credit_officer", "ops_manager", "it_admin")),
    ...
    await AuditService.log(
        ...
        description=(f"AI assessment: grade {assessment['risk_grade']} "
                     f"PD {assessment['pd']:.1%} rec {assessment['recommendation']}"),
        payload={"model_version": assessment["model_version"],
                 "risk_grade": assessment["risk_grade"], "pd": assessment["pd"]},
    )
