...
def _valid_app(**overrides):
    base = {
        ...
        "sector": "IT",
        "director_name": "Jane",
        "director_email": "jane@acme.ie",
        "director_phone": "+353871234567",
        "address": "Dublin 2",
        "years_trading": "5-10",
...
def test_decision_valid():
    d = DecisionCreate(
        outcome="approved", rationale="Strong financials, DSCR above policy minimum."
    )
...
def test_decision_invalid_outcome_fails():
    with pytest.raises(Exception):
        DecisionCreate(
            outcome="maybe", rationale="This is a valid rationale length okay."
        )
