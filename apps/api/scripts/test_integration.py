...
from app.core.database import Base, make_engine  # noqa: E402
from app.models.application import Application, ApplicationStatus  # noqa: E402
from app.services.ai.assessment_service import apply_to_orm, assess  # noqa: E402

ASSESS_FIELDS = ("sector", "years_trading", "annual_revenue", "net_profit", "total_assets",
                 "total_liabilities", "existing_debt", "monthly_repayment", "loan_amount",
                 "loan_term_months", "loan_purpose", "has_collateral")

SAMPLE = dict(
    company_name="Shannon Logistics Ltd", crn="512345", sector="Logistics",
    director_name="Aoife Murphy", director_email="aoife@shannon.ie",
    director_phone="+353 87 123 4567", address="14 Quay Road, Shannon",
    years_trading="12", loan_amount=150_000, loan_purpose="Working Capital",
    loan_term_months=60, purpose_detail="Fund seasonal working capital and a new vehicle.",
    has_collateral=True, annual_revenue=2_400_000, net_profit=240_000,
    total_assets=1_800_000, total_liabilities=600_000, existing_debt=250_000,
...
async def main() -> int:
    ...
    try:
        ...
        async with Session() as s:
            row = Application(reference=ref, status=ApplicationStatus.submitted, **SAMPLE)
            s.add(row)
            await s.flush()
            app_id = row.id
            ...
            await s.commit()
            print(f"[B+C] assessed: grade {result['risk_grade']} | "
                  f"PD {result['pd']:.1%} | rec {result['recommendation'].upper()}")

        # read back in a fresh session -> proves persistence
        async with Session() as s:
            row = (await s.execute(
                select(Application).where(Application.id == app_id))).scalar_one()
            print("\n[verify] AI fields read back from DB:")
            for f in ("pd", "risk_grade", "ai_score", "dscr", "apr", "affordability",
                      "ecl_12m", "ifrs9_stage", "model_version", "status"):
