...
from app.services.ai.worker import process_job  # noqa: E402
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        app_row = Application(
            reference=ref, status=ApplicationStatus.draft,
            company_name="Placeholder Ltd", crn="000000", sector="Retail",
            director_name="N A", director_email="n@a.ie", director_phone="+353 0",
            address="1 Test St", years_trading="1", loan_amount=50_000,
            loan_purpose="Working Capital", loan_term_months=36, purpose_detail="test",
            has_collateral=False,
        )
        db.add(app_row)
        await db.flush()
        app_id = app_row.id
        db.add(Document(application_id=app_id, doc_type="FINANCIALS",
                        filename=SAMPLE_PDF.name, s3_key=str(SAMPLE_PDF),
                        content_type="application/pdf", size_bytes=SAMPLE_PDF.stat().st_size))
        job = ProcessingJob(application_id=app_id, status=JobStatus.queued, progress=0, steps_log=[])
        db.add(job)
        await db.commit()
        job_id = job.id
    ...
    async with AsyncSessionLocal() as db:
        ...
        app_row = (await db.execute(select(Application).where(Application.id == app_id))).scalar_one()
        ...
        print(f"extracted    : {app_row.company_name} | {app_row.sector} | "
              f"revenue €{float(app_row.annual_revenue or 0):,.0f}")
        print(f"assessment   : grade {app_row.risk_grade} | PD {float(app_row.pd or 0):.1%} | "
              f"DSCR {app_row.dscr} | IFRS9 stage {app_row.ifrs9_stage} | "
              f"ECL €{float(app_row.ecl_12m or 0):,.0f}")

        ok = (job.status == JobStatus.completed and job.progress == 100
              ...
              and app_row.company_name != "Placeholder Ltd")   # proves extraction overwrote it

        # cleanup
        await db.execute(delete(Document).where(Document.application_id == app_id))
        await db.execute(delete(ProcessingJob).where(ProcessingJob.application_id == app_id))
